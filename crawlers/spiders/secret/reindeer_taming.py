...
class ReindeerTamingBot(scrapy.Spider):
    '''
    @sumamry: Our ReindeerBot has crawled the pages of the rekhta.org very well. 
    But we found that lines of the crawled poetry are shuffled (rekhta's trick).
    Here we will tame our Reindeer, to bring out poetries with correct line order.
    ...
    def parse(self, response):
        ...
        for alphabet in ['a']:#FIXME TESTING: change back to ABCD after testing
...
#                     yield scrapy.Request(url_t, callback=self.l4_parse_poetry)                          
    ...
    def l4_parse_poetry(self, response):
        ...
        try:
            ...
            image = response.xpath("//div[@id='ImageHost']//div[@class='PoemDisplay']//img/@src").extract()[0]
            ...
            # Make a list of lines of poem
            poem_lines = poem.split('<br>')

            # Get the poem image

            # Convert poem image to text using OCR

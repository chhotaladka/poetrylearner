...
@register.simple_tag
def generic_prop_list(namespace, name, lst):
    """
    Generic property setter that allows to create custom namespaced meta
    """
    return '<meta property="%s:%s" content="%s">' % (namespace, name,  ' '.join(lst))
...
@register.simple_tag
def og_prop_list(name, lst):
    try:
        return '<meta property="og:%s" content="%s">' % (name, ' '.join(lst))
    except:
        return ' '
...
@register.simple_tag
def meta_tags(name, value):
    return '<meta name="%s" content="%s">' % (name, value)

@register.simple_tag
def meta_rel_tags(name, value):
    return '<link rel="%s" href="%s"/>' % (name, value)


@register.simple_tag
def meta_tag_list(name, lst):
    try:
